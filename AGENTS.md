...
| [API-DISCREPANCIES.md](API-DISCREPANCIES.md)         | every place the docs and the live API disagree                   |
...
- Before trusting a response shape you have not seen on the wire, check
  [API-DISCREPANCIES.md](API-DISCREPANCIES.md) — and add to it when you find a
  new one.
...
- `SURFACE.txt` must match. `make surface-snapshot`, committed in the same
  change.
...
targets 1.24, but a 1.25-built linter still cannot read a 1.26 stdlib.
`mise install` gets a matching build.
