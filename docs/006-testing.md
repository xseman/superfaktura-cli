...
1.25-built linter still cannot read a 1.26 stdlib. Use a current release
binary, or build one with your own toolchain:
...
`SURFACE.txt` records every command, flag and positional argument. The file is
untracked: `make surface-snapshot` writes it, and from then on the test diffs
the surface against it — so a rename shows up as a readable diff before it
breaks somebody's script. Without the file the test skips.
