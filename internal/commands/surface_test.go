...
// SURFACE.txt is a snapshot of the command tree: every command, subcommand,
// flag and positional argument. It is not documentation — it is a tripwire.
// Renaming a flag or dropping a subcommand breaks somebody's script, so that
// change has to be deliberate and visible in a diff.
//
// The file is untracked: `make surface-snapshot` writes it, and from then on
// this test diffs the surface against it. Without the file the test skips.
...
func TestSurfaceSnapshot(t *testing.T) {
	...
	if os.IsNotExist(err) {
		t.Skipf("%s not present; write it with: make surface-snapshot", surfaceFile)
	}
	if err != nil {
		t.Fatalf("read %s: %v", surfaceFile, err)
	}
	...
	report.WriteString("\nIf this is intended, refresh it with: make surface-snapshot")
	if len(removed) > 0 {
