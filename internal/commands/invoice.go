...
difference between one request and a hundred out of a daily allowance of 1000.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(ids) == 1 {
				...
				return emitDetail(raw, obj, "Invoice.id", invoiceDetail())
			}
...
available for about three months.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// detail. Unwrap so the same field list renders it — the raw bytes
			// too, so --json shows the same record the table does.
