...
//
// Raw holds each item's original bytes, aligned with Items. The maps drive
// tables, filters and id lookups; the raw bytes are what --json emits, so the
// API's own key order survives to the consumer. Raw stays empty when the rows
// were synthesized rather than received (keyed fallbacks inject ids).
...
func fetchList(ctx context.Context, path string, opts *listOptions, extra client.Params) (listResult, error) {
	...
	for {
		...
		combined.Raw = append(combined.Raw, result.Raw...)
...
func decodeList(raw json.RawMessage) (listResult, error) {
	...
	if err := json.Unmarshal(raw, &wrapped); err == nil && wrapped.Items != nil {
		if items, ok := decodeItems(wrapped.Items); ok {
			return listResult{
				Items:     items,
				Raw:       wrapped.Items,
				ItemCount: wrapped.ItemCount,
	...
	if err := json.Unmarshal(raw, &bare); err == nil {
		if items, ok := decodeItems(bare); ok {
			return listResult{Items: items, Raw: bare, ItemCount: len(items), PageCount: 1}, nil
		}
	}
...
// decodeItems decodes every row, or reports that the shape is not a list of
// records so the caller can try the next one.
func decodeItems(raw []json.RawMessage) ([]map[string]any, bool) {
...
func emitList(result listResult, cols []render.Column, next ...output.Step) error {
	...
	// The maps rendered the table; the machine formats get the API's own
	// bytes, so key order reaches the consumer exactly as it arrived.
...
// emitObject prints one record. raw is the record's original bytes; the map
// renders the human view, the bytes are what --json emits.
func emitObject(raw json.RawMessage, obj map[string]any, fields []render.Field) error {
	idPath := ""
	if len(fields) > 0 {
		idPath = fields[0].Path
	}
	return emitDetail(raw, obj, idPath, render.Pairs(nil, fields))
}
...
func emitDetail(raw json.RawMessage, obj map[string]any, idPath string, show render.Renderer) error {
	...
	return emit(data, func(w io.Writer) {
...
func emitWrite(raw json.RawMessage, model, message string, next ...output.Step) error {
	...
	return emit(raw, func(w io.Writer) {
