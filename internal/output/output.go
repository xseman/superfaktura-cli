...
func (w *Writer) writeIDs(data any) error {
	// Raw bytes are decoded here on demand; everywhere else they pass through
	// untouched.
	switch typed := data.(type) {
	case json.RawMessage:
		var record map[string]any
		if json.Unmarshal(typed, &record) != nil {
			return nil
		}
		return w.writeIDs(record)
	case []json.RawMessage:
...
// NormalizeData shapes a value for the envelope. Raw JSON passes through
// verbatim — the API's own key order is the output contract, because a
// re-marshal through Go maps re-sorts every object and floods a consumer's
// diffs with reshuffles. Typed structs round-trip to plain maps and slices;
// numbers are preserved exactly rather than being widened to float64, which
// would turn an invoice ID into 1.042e+03.
